...
// Helper function to upload files with proper typing for Cypress
declare global {
  namespace Cypress {
    interface Chainable {
...
(Cypress.Commands as any).add('uploadFile', { prevSubject: true } as any, (subject: JQuery<HTMLInputElement>, fileName: string, mimeType: string) => {
  cy.fixture(fileName, 'base64').then((fileContent) => {
    ...
    (el as any).files = dataTransfer.files;
    cy.wrap(subject).trigger('change', { force: true });
  });
});

// Helper to check accessibility
(Cypress.Commands as any).add('checkA11y', (context?: any, options?: any) => {
  const anyCy = cy as any;
  if (typeof anyCy.injectAxe === 'function') {
    anyCy.injectAxe();
  }
  if (typeof anyCy.checkA11y === 'function') {
    anyCy.checkA11y(context, options);
  }
});
