...
export default function PlayerDashboard() {
  ...
  const error = null;
  const isSubmittingWellness = false;
  ...
  const readiness = historicalWellnessData.slice(-5);
  ...
  const handleWellnessSubmit = async () => {
    try {
      ...
      console.log("Wellness submitted:", wellnessForm);
    } catch (error) {
  ...
  const handleTrainingComplete = async (trainingTitle: string) => {
    try {
      ...
      console.log(`Completed ${trainingTitle}`);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <DashboardHeader 
        title={`${playerInfo.name} Dashboard`}
        subtitle={`#${playerInfo.number} • ${playerInfo.position} • ${playerInfo.team}`}
        role="player"
      />
        ...
        <TabsContent value="training" className={spacing.card} role="tabpanel" aria-labelledby="training-tab">
            ...
            <CardHeader>
              ...
              <CardContent>
                <div className="space-y-4">
                  {training.map((t: any, index: number) => (
          ...
          <Card>
            <CardHeader>
              ...
              <CardContent>
                <div className="space-y-4">
                  {playerDevelopmentGoals.map((goal: any, index: number) => (
            ...
            <Card>
              ...
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {wellnessStats.recommendations.map((rec: string, index: number) => (
