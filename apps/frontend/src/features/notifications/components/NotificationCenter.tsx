...
export function NotificationCenter({ userId, organizationId, className }: NotificationCenterProps) {
  ...
  const handleBulkMarkAsRead = async () => {
    if (selectedNotifications.length > 0 && markMultipleAsRead) {
  ...
  const getTabCount = (tab: string) => {
    ...
    switch (tab) {
      case 'unread':
        return notificationList.filter(n => n && !n.read_at).length;
      case 'important':
        return notificationList.filter(n => n && (n.priority === 'high' || n.priority === 'urgent')).length;
      case 'calendar':
        return notificationList.filter(n => 
          n && n.type && (n.type.includes('event') || n.type.includes('rsvp') || n.type.includes('schedule'))
        ).length;
      case 'training':
        return notificationList.filter(n => n && n.type && n.type.includes('training')).length;
      case 'medical':
        return notificationList.filter(n => 
          n && n.type && (n.type.includes('medical') || n.type.includes('injury'))
        ).length;
      default:
                  ...
                  <div className="divide-y">
                    {filteredNotifications.map((notification) => (
                      <NotificationItem
                        ...
                        onMarkAsRead={() => markAsRead && markAsRead(notification.id)}
                        onDelete={() => deleteNotification && deleteNotification(notification.id)}
                      />
                    ))}
                  </div>
                )}
