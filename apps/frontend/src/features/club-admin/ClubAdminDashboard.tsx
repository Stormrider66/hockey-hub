...
// Fallback: use dashboardApi as placeholder for club overview to unblock build
...
export default function ClubAdminDashboard() {
  ...
  const { data: apiData, isLoading } = useGetUserDashboardDataQuery();

  const orgStats = (apiData as any)?.orgStats ?? { teams: 8, activeMembers: 243, coachingStaff: 18, upcomingEvents: 28 };

  const teamsList = (apiData as any)?.teams ?? [
  ...
  const roleBreakdown = (apiData as any)?.roleBreakdown ?? [
    { name: "Players", value: 180, color: "#3b82f6" },
    { name: "Coaches", value: 15, color: "#10b981" },
    { name: "Parents", value: 35, color: "#6366f1" },
  ];

  const members = (apiData as any)?.members ?? [];
  const tasks = (apiData as any)?.tasks ?? [];

  const eventsByDate = React.useMemo(() => {
    const evts = (apiData as any)?.events ?? [];
