...
export const canViewEvent = (event: CalendarEvent, user: UserPermissions): boolean => {
  ...
  if (event.participants?.some((p: any) => (p.userId ?? p.participantId) === user.userId)) {
    return true;
  }
  ...
  switch (event.visibility) {
    ...
    case EventVisibility.ROLE_BASED:
      ...
      if ((event as any).visibleToRoles?.includes(user.role)) {
        return true;
      }
      break;
  }
...
export const canEditEvent = (event: CalendarEvent, user: UserPermissions): boolean => {
  ...
  const participant: any = event.participants?.find((p: any) => (p.userId ?? p.participantId) === user.userId);
