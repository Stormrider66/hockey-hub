...
export const SystemAnnouncementBanner: React.FC = () => {
  ...
  const bannerAnnouncements = data?.announcements?.filter(({ announcement, recipientStatus }) => {
    const show = Boolean(announcement.metadata?.show_banner) && announcement.status === 'sent';
    ...
    const notDismissed = recipientStatus !== 'dismissed' && !dismissedBanners.has(announcement.id);
    return show && notExpired && notDismissed;
  }) || [];
