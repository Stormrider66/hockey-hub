...
const HOCKEY_EQUIPMENT_OPTIONS: AgilityEquipmentType[] = [
  'hockey_stick', 'pucks', 'shooting_targets', 'passing_targets', 'shooting_board', 
  'synthetic_ice', 'hockey_net', 'hockey_pads', 'hockey_pylons'
];
...
const HOCKEY_PATTERN_OPTIONS: DrillPattern[] = [
  'hockey_stop_start', 'breakout_pattern', 'shooting_drill', 'passing_drill', 'deke_pattern', 'edge_work_pattern'
];
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab}>
          ...
          <TabsContent value="basic" className="space-y-4">
            ...
            <div>
              ...
              <div className="space-y-3 mt-2">
                {/* Standard Equipment */}
                <div>
                  <h4 className="text-sm font-medium mb-2">Standard Equipment</h4>
                ...
                {/* Hockey Equipment */}
                {editedDrill.category === 'sport_specific' && (
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {HOCKEY_EQUIPMENT_OPTIONS.map(equipment => (
                        <Badge
                          key={equipment}
                          variant={editedDrill.equipment.includes(equipment) ? "default" : "outline"}
                          className="cursor-pointer"
                          onClick={() => toggleEquipment(equipment)}
                        >
                          {equipment === 'hockey_stick' ? 'Hockey Stick' : 
                           equipment === 'hockey_net' ? 'Hockey Net' : 
                           equipment === 'hockey_pylons' ? 'Hockey Pylons' :
                           equipment === 'shooting_targets' ? 'Shooting Targets' :
                           equipment === 'passing_targets' ? 'Passing Targets' :
                           equipment === 'shooting_board' ? 'Shooting Board' :
                           equipment === 'synthetic_ice' ? 'Synthetic Ice' :
                           equipment === 'hockey_pads' ? 'Hockey Pads' :
                           equipment === 'pucks' ? 'Pucks' :
                           equipment.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                        </Badge>
                      ))}
                    </div>
          ...
          <TabsContent value="metrics" className="space-y-4">
                ...
                <div className="space-y-3 mt-2">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">Shot Speed (mph/kph)</span>
                    </div>
                    <Switch
                      checked={editedDrill.metrics.shotSpeed || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, shotSpeed: checked } 
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      checked={editedDrill.metrics.shotAccuracy || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, shotAccuracy: checked } 
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">Pass Completion Rate (%)</span>
                    </div>
                    <Switch
                      checked={editedDrill.metrics.passCompletionRate || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, passCompletionRate: checked } 
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">Pass Speed</span>
                    </div>
                    <Switch
                      checked={editedDrill.metrics.passSpeed || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, passSpeed: checked } 
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">Puck Control Time</span>
                    </div>
                    <Switch
                      checked={editedDrill.metrics.puckControlTime || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, puckControlTime: checked } 
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <Users className="h-4 w-4 text-muted-foreground" />
                      <span className="text-sm">Deke Success Rate (%)</span>
                    </div>
                    <Switch
                      checked={editedDrill.metrics.dekeSuccessRate || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, dekeSuccessRate: checked } 
                        })
                      }
                    />
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm">Decision Making Time</span>
                    </div>
                    <Switch
                      checked={editedDrill.metrics.decisionMakingTime || false}
                      onCheckedChange={(checked) => 
                        updateDrill({ 
                          metrics: { ...editedDrill.metrics, decisionMakingTime: checked } 
                        })
                      }
                    />
                  </div>
            ...
            <Alert>
              ...
              <p className="text-sm">
                {editedDrill.category === 'sport_specific' 
                  ? 'Metrics will be tracked during drill execution. Hockey-specific metrics like shot speed and accuracy require appropriate measurement equipment. Players can record their performance and track improvement over time.'
                  : 'Metrics will be tracked during drill execution. Players can record their performance and track improvement over time.'
                }
              </p>
            </Alert>
          </TabsContent>
