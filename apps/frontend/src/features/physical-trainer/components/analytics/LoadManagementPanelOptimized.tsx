...
export function LoadManagementPanelOptimized({
  players,
  loadData,
  filters,
  isLoading,
  ...
  // Format load history for charts
  const formattedLoadHistory = useMemo(() => {
    if (!selectedPlayerData) return [];
    
    return selectedPlayerData.loadHistory.slice(-28).map(point => ({
      ...point,
      ...
      <Tabs value={viewMode} onValueChange={(value: any) => setViewMode(value)}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="lg:col-span-2">
                  ...
                  <CardContent>
                    <div className="h-80">
                      <SimpleLineChart
                        data={formattedLoadHistory}
                        height={320}
                        lines={[
                          { dataKey: 'ratio', stroke: '#3b82f6', strokeWidth: 2, name: 'A:C Ratio' },
                          { dataKey: 'wellness', stroke: '#10b981', strokeWidth: 2, name: 'Wellness' }
                        ]}
                      />
                      {/* Note: Reference lines for risk thresholds would need to be displayed differently */}
                      <div className="mt-2 text-xs text-muted-foreground text-center">
                        Red line: High Risk (1.3) | Orange line: Moderate Risk (1.1)
                      </div>
                    </div>
                  </CardContent>
                </Card>
        ...
        <TabsContent value="team" className="mt-6">
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                {/* Area chart for average ratio */}
                <div className="h-64">
                  <SimpleAreaChart
                    data={teamOverviewData}
                    height={256}
                    areas={[
                      { dataKey: 'avgRatio', stroke: '#3b82f6', fill: '#3b82f6', fillOpacity: 0.3, name: 'Average A:C Ratio' }
                    ]}
                  />
                </div>
                ...
                <div className="h-64">
                  <SimpleBarChart
                    ...
                    height={256}
                    bars={[
                      { dataKey: 'highRiskCount', fill: '#ef4444', name: 'High Risk Players' },
                      { dataKey: 'moderateRiskCount', fill: '#f59e0b', name: 'Moderate Risk Players' }
                    ]}
                  />
                </div>
              </div>
              <div className="mt-2 text-xs text-muted-foreground text-center">
                Red line: High Risk threshold (1.3) | Orange line: Moderate Risk threshold (1.1)
              </div>
            </CardContent>
          </Card>
        </TabsContent>
