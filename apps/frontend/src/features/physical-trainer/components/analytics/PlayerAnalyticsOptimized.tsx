...
import { LoadManagementPanelOptimized } from './LoadManagementPanelOptimized';
import { 
  SimpleRadarChart, 
  SimpleLineChart, 
  SimpleBarChart,
...
export function PlayerAnalyticsOptimized({
  players = [],
  loadManagement = [],
  recovery = [],
  isLoading,
  ...
  // Prepare radar data for multiple players
  const multiPlayerRadarData = selectedPlayers.slice(0, 4).map((player) => ({
    name: player.playerName,
    dataKey: 'value',
    stroke: `hsl(${selectedPlayers.indexOf(player) * 90}, 70%, 50%)`,
    fill: `hsl(${selectedPlayers.indexOf(player) * 90}, 70%, 50%)`,
    fillOpacity: 0.1,
    strokeWidth: 2
  }));
              ...
              <Card>
                ...
                <CardContent>
                  <SimpleComposedChart
                    ...
                    xAxisDataKey="date"
                    leftYAxisConfig={{ orientation: 'left' }}
                    rightYAxisConfig={{ orientation: 'right' }}
                    showLegend={true}
                  />
                </CardContent>
              </Card>
