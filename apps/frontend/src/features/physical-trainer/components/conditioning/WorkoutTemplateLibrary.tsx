...
import type { WorkoutTemplate, IntervalSet } from '../../types';
import { 
  WorkoutEquipmentType,
  ConditioningMode,
  generateModeTemplates
...
export default function WorkoutTemplateLibrary({
  ...
  const modeTemplates = generateModeTemplates(mode);
  const allTemplates = [...WORKOUT_TEMPLATES, ...modeTemplates];
  ...
  const filteredTemplates = allTemplates.filter(template => {
    ...
    const matchesMode = !mode || !template.mode || template.mode === mode;
    
    return matchesSearch && matchesCategory && matchesEquipment && matchesMode;
  });
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      ...
      case 'active_recovery': return <span className="text-green-500">🧘</span>;
      case 'hrv_guided': return <Heart className="h-4 w-4 text-green-500" />;
      case 'sprint_intervals': return <span className="text-purple-500">⚡</span>;
      case 'hill_repeats': return <span className="text-purple-500">⛰️</span>;
      ...
      <div className="space-y-4 mb-4">
        ...
        <Tabs value={selectedCategory} onValueChange={setSelectedCategory}>
          <TabsList className="grid w-full grid-cols-7">
            ...
            <TabsTrigger value="active_recovery">Recovery</TabsTrigger>
            <TabsTrigger value="sprint_intervals">Sprint</TabsTrigger>
