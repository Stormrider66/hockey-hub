...
describe('dashboardApi', () => {
  ...
  describe('getStatisticsSummary', () => {
    ...
    it('should use different cache times based on user type', () => {
      const endpoint = dashboardApi.endpoints.getStatisticsSummary;
      
      // Admin cache time should be 600 seconds
      expect(endpoint.keepUnusedDataFor('admin' as any)).toBe(600);
      
      // Coach cache time should be 300 seconds
      expect(endpoint.keepUnusedDataFor('coach' as any)).toBe(300);
      
      // Player/trainer cache time should be 180 seconds
      expect(endpoint.keepUnusedDataFor('player' as any)).toBe(180);
    });
  });
