...
} from '@/features/physical-trainer/types/validation-api.types';
...
export const trainingApi = createApi({
  ...
  tagTypes: ['Session', 'Test', 'TestBatch', 'Exercise', 'Template', 'Execution', 'ValidationRules'],
  endpoints: (builder) => ({
    ...
    getExercises: builder.query<{ exercises: ExerciseTemplate[] }, { category?: string; search?: string; organizationId?: string }>({
      ...
      providesTags: (result) => 
        result?.exercises
          ...
          : [{ type: 'Exercise', id: 'LIST' }],
      keepUnusedDataFor: 900, // 15 minutes cache for exercises (rarely change)
    ...
    // Test Sessions
    getTestSessions: builder.query<TestSession[], { status?: string; date?: string }>({
      query: (params) => ({
        url: '/test-sessions',
        params,
      }),
    ...
    updateTestSession: builder.mutation<TestSession, { id: string; data: Partial<CreateTestSessionRequest> }>({
      query: ({ id, data }) => ({
    ...
    // ========================================================================
    // Unified Validation Endpoints
    // ========================================================================

    // Full workout validation
    validateWorkout: builder.mutation<ValidationResponse, ValidationRequest>({
      query: (request) => ({
        url: '/validation/workout',
        method: 'POST',
        body: request,
      }),
    }),
    ...
    validateWorkoutContent: builder.mutation<ContentValidationResponse, ContentValidationRequest>({
      query: (request) => ({
        url: '/validation/content',
        method: 'POST',
        body: request,
      }),
    }),
    ...
    validatePlayerAssignments: builder.mutation<AssignmentValidationResponse, AssignmentValidationRequest>({
      query: (request) => ({
        url: '/validation/assignments',
        method: 'POST',
        body: request,
      }),
    }),
    ...
    validateMedicalCompliance: builder.mutation<MedicalValidationResponse, MedicalValidationRequest>({
      query: (request) => ({
        url: '/validation/medical',
        method: 'POST',
        body: request,
      }),
    }),
    ...
    validateSchedule: builder.mutation<ScheduleValidationResponse, ScheduleValidationRequest>({
      query: (request) => ({
    ...
    // Batch validation
    validateBatch: builder.mutation<BatchValidationResponse, BatchValidationRequest>({
      query: (request) => ({
    ...
    getValidationRules: builder.query<ApiResponse<{ rules: ValidationRule[] }>, { 
      workoutType?: string; 
      category?: string; 
      enabled?: boolean 
    }>({
      ...
      providesTags: ['ValidationRules'],
    }),

    // Update validation configuration
    updateValidationConfig: builder.mutation<ApiResponse<{ success: boolean }>, RealTimeValidationConfig>({
      query: (config) => ({
        url: '/validation/config',
        method: 'PUT',
        body: config,
      }),
      invalidatesTags: ['ValidationRules'],
    }),

    // Get validation configuration
    getValidationConfig: builder.query<ApiResponse<RealTimeValidationConfig>, void>({
      query: () => '/validation/config',
      providesTags: ['ValidationRules'],
    }),

    // Real-time field validation
    validateField: builder.mutation<{
      isValid: boolean;
    ...
    // Cross-field validation (for dependencies between fields)
    validateFieldDependencies: builder.mutation<{
      isValid: boolean;
      ...
      query: (request) => ({
        url: '/validation/field-dependencies',
        method: 'POST',
        body: request,
      }),
    }),

    // Progressive validation (validate as user builds workout)
    validateProgressive: builder.mutation<{
      currentStep: string;
      isValid: boolean;
      canProceed: boolean;
      errors: Array<{ step: string; field: string; message: string }>;
      warnings: Array<{ step: string; field: string; message: string }>;
    ...
    validateWithSuggestions: builder.mutation<{
    ...
    }, ValidationRequest & { includeSuggestions: true }>({
      query: (request) => ({
...
// Manually create lazy query hooks
export const useLazyGetWorkoutSessionByIdQuery = trainingApi.endpoints.getWorkoutSessionById.useLazyQuery;
export const useLazyGetConditioningWorkoutByIdQuery = trainingApi.endpoints.getConditioningWorkoutById.useLazyQuery;
export const useLazyGetHybridWorkoutByIdQuery = trainingApi.endpoints.getHybridWorkoutById.useLazyQuery;
export const useLazyGetAgilityWorkoutByIdQuery = trainingApi.endpoints.getAgilityWorkoutById.useLazyQuery;