...
const FileManager: React.FC<FileManagerProps> = ({
  ...
  const handleFileSelect = (file: FileData) => {
    if (selectable) {
      ...
      if (onFilesSelect) {
        const selectedRaw = data?.files.filter(f => newSelectedFiles.has(f.id)) || [];
        const selected: FileData[] = selectedRaw.map((f) => ({
          id: f.id,
          name: f.name,
          size: f.size,
          url: f.url,
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      ...
      if (onFilesSelect) {
        const mapped: FileData[] = (data?.files || []).map((f) => ({
          id: f.id,
          name: f.name,
          size: f.size,
          url: f.url,
    ...
    <div className={cn('space-y-4', className)}>
      {showFilters && (
        <div className="flex flex-col sm:flex-row gap-4">
          ...
          <Select value={sortBy} onValueChange={(value: string) => setSortBy(value as 'createdAt' | 'name' | 'size')}>
