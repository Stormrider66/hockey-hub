...
export class CachedPlanningService {
  ...
  async getCoachDashboardData(coachId: string, organizationId: string) {
    ...
    return {
      activePlans: activePlans.map((plan: any) => ({
        id: plan.id || (plan as any).id,
        name: plan.name,
        teamId: plan.teamId,
        progress: plan.getProgress(),
        currentPhase: plan.getCurrentPhase(this.getCurrentWeek(plan.startDate))
      })),
      upcomingPractices: upcomingPractices.map((practice: any) => ({
        id: practice.id || (practice as any).id,
      ...
      popularDrills: popularDrills.map((drill: any) => ({
        id: drill.id || (drill as any).id,
        name: drill.name,
        type: drill.type,
  ...
  async getPlayerDashboardData(playerId: string, teamId: string) {
    ...
    return {
      upcomingPractices: upcomingPractices.slice(0, 5).map((practice: any) => ({
        id: practice.id || (practice as any).id,
        title: practice.title,
        date: practice.date,
        duration: practice.duration,
  ...
  async updatePracticePlan(practiceId: string, data: any) {
    return this.planRepo.updatePracticePlan(practiceId, data);
  }

  async getUpcomingPractices(teamId: string, days: number = 7) {
    return this.planRepo.findUpcomingPractices(teamId, days);
  }
  ...
  async useTemplate(templateId: string, teamId: string, coachId: string) {
    ...
    const trainingPlan = await this.planRepo.createTrainingPlan({
      ...
      startDate,
      endDate,
      goals: (template as any).goals as any,
  ...
  // Analytics
  async getPlanningAnalytics(organizationId: string, _startDate?: Date, _endDate?: Date) {
