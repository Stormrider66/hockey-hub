...
describe('Chat API Integration Tests', () => {
  ...
  describe('WebSocket Integration', () => {
    ...
    let testConversation: any;
    const serverAddress = httpServer.address() as any;
    const socketUrl = `http://localhost:${serverAddress.port}`;
    ...
    it('should handle real-time message delivery', async (done) => {
      // Connect both users
    ...
    it('should handle message read receipts', async (done) => {
    ...
    it('should handle presence updates', async (done) => {
      clientSocket1 = ioClient(socketUrl, {
        auth: { token: authTokens.user1 },
      });
