...
const logger = new Logger('cached-message-repo');
...
export class CachedMessageRepository {
  ...
  // Message retrieval
  async findById(id: string, useCache = true): Promise<Message | null> {
  ...
  async searchMessages(
    ...
    try {
      const cached = await redisClient.get(cacheKey);
      if (cached) {
        logger.debug(`Cache hit for message search: ${query}`);
        return JSON.parse(cached);
      }
    } catch (error) {
      logger.warn('Redis get error for message search:', error);
    }
    ...
    try {
      ...
      logger.warn('Redis set error for message search:', error);
    }
