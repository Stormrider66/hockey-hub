 ...
 */
router.post('/initialize', extractUser, requireAuth, async (req, res, next) => {
  try {
 ...
 *         schema:
 *           type: string
 *           enum: [STRENGTH, CARDIO, AGILITY, FLEXIBILITY, POWER, ENDURANCE, RECOVERY, REHABILITATION, SPORT_SPECIFIC, MENTAL, HYBRID, STABILITY_CORE, PLYOMETRICS, WRESTLING]
 ...
 */
router.get('/', extractUser, requireAuth, async (req, res, next) => {
  try {
    const organizationId = req.user!.organizationId;
 ...
 */
router.get('/statistics', extractUser, requireAuth, async (req, res, next) => {
  try {
    const organizationId = req.user!.organizationId;
 ...
 */
router.get('/:workoutType', extractUser, requireAuth, async (req, res, next) => {
  try {
    const organizationId = req.user!.organizationId;
 ...
 *         description: Configuration not found
 */
router.delete('/:workoutType', extractUser, requireAuth, async (req, res, next) => {
  try {
    const organizationId = req.user!.organizationId;
    const workoutType = req.params.workoutType as WorkoutType;
 ...
 */
router.get('/:workoutType/progression/:level', extractUser, requireAuth, async (req, res, next) => {
  try {
    const organizationId = req.user!.organizationId;
    const workoutType = req.params.workoutType as WorkoutType;
