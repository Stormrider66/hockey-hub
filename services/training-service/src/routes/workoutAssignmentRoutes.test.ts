...
describe('Workout Assignment Routes', () => {
  ...
  beforeEach(() => {
    ...
    // Ensure router uses our mocked instance as well
    (global as any).__assignmentService = mockAssignmentService;

    // Require routes after mocks so router picks up mocked instance
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    const routes = require('./workoutAssignmentRoutes').default;
    app.use('/api/v1/training/workouts', routes);
  });
