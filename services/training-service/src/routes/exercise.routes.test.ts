...
// Require routes after mocks so auth injection is applied
...
describe('Exercise Routes', () => {
  ...
  beforeEach(() => {
    ...
    // Inject a deterministic mocked instance for router to use
    (global as any).__exerciseService = new ExerciseService() as jest.Mocked<ExerciseService>;
    const { exerciseRoutes } = require('./exercise.routes');
    app.use('/api/v1/training', exerciseRoutes);
    
    mockExerciseService = (global as any).__exerciseService as jest.Mocked<ExerciseService>;
  });
  ...
  describe('GET /api/v1/training/exercises/:id', () => {
    ...
    it('should return 404 for non-existent exercise', async () => {
      mockExerciseService.findById = jest.fn().mockResolvedValue(undefined as any);

      const response = await request(app)
        .get('/api/v1/training/exercises/999')
        .expect(404);

      expect(response.body).toEqual({ success: false, error: 'Exercise not found' });
    });
  });
