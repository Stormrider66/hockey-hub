...
exports.CachedWorkoutSessionService = void 0;
const database_1 = require("../config/database");
const entities_1 = require("../entities");
const CachedWorkoutSessionRepository_1 = require("../repositories/CachedWorkoutSessionRepository");
const shared_lib_1 = require("@hockey-hub/shared-lib");
const exerciseRepository = () => database_1.AppDataSource.getRepository(entities_1.Exercise);
const playerLoadRepository = () => database_1.AppDataSource.getRepository(entities_1.PlayerWorkoutLoad);
class CachedWorkoutSessionService {
    constructor() {
        this.workoutRepository = new CachedWorkoutSessionRepository_1.CachedWorkoutSessionRepository();
        this.cacheManager = shared_lib_1.RedisCacheManager.getInstance();
    }
    async createWorkoutSession(data) {
        ...
        if (data.exercises && data.exercises.length > 0) {
            const workoutExercises = data.exercises.map((ex, index) => exerciseRepository().create({
                ...ex,
        ...
        if (data.playerLoads && data.playerLoads.length > 0) {
            const loads = data.playerLoads.map(load => playerLoadRepository().create({
                ...load,
                workoutSessionId: savedWorkout.id
            }));
    ...
    async updateWorkoutSession(id, data) {
        ...
        if (data.playerIds)
            workout.playerIds = data.playerIds;
        if (data.settings)
            workout.settings = { ...workout.settings, ...data.settings };
    ...
    async getWorkoutSessions(filters, page = 1, limit = 20) {
        // Use cached repository for common queries
        if (filters.teamId && !filters.playerId && !filters.date && !filters.status) {
            const [sessions, total] = await this.workoutRepository.findSessionsByTeam(filters.teamId, page, limit);
            return {
                data: sessions,
                pagination: {
                    page,
        ...
        if (filters.playerId && !filters.date && !filters.status) {
            const [sessions, total] = await this.workoutRepository.findSessionsByPlayer(filters.playerId, page, limit);
            return {
                data: sessions,
                pagination: {
                    page,
        ...
        if (filters.date) {
            const sessions = await this.workoutRepository.findSessionsByDate(filters.date, filters.teamId);
        ...
        if (filters.status) {
            const sessions = await this.workoutRepository.findSessionsByStatus(filters.status, filters.teamId, 100 // Get more for status queries
            );
            // Apply pagination
            const startIndex = (page - 1) * limit;
            const endIndex = startIndex + limit;
    ...
    async getUpcomingWorkoutSessions(playerId, teamId, days = 7) {
        return this.workoutRepository.findUpcomingSessions(playerId, teamId, days);
    }
    async updatePlayerWorkoutLoad(sessionId, playerId, data) {
        ...
        if (!playerLoad) {
            ...
            if (data.loadModifier !== undefined)
                playerLoad.loadModifier = data.loadModifier;
            if (data.exerciseModifications)
                playerLoad.exerciseModifications = data.exerciseModifications;
            if (data.notes !== undefined)
                playerLoad.notes = data.notes;
        }
        const savedLoad = await playerLoadRepository().save(playerLoad);
        // Invalidate session cache
        await this.cacheManager.delete(shared_lib_1.CacheKeyBuilder.build('workout-session', 'complete', sessionId));
        return savedLoad;
    }
    async getPlayerWorkoutLoad(sessionId, playerId) {
        const cacheKey = shared_lib_1.CacheKeyBuilder.build('player-load', sessionId, playerId);
        ...
        if (playerLoad) {
            ...
            await this.cacheManager.set(cacheKey, playerLoad, 600, [`player:${playerId}`, `workout-session:${sessionId}`]);
        }
        return playerLoad;
    }
    async getWorkoutSessionsByTeamAndDateRange(teamId, startDate, endDate) {
        const cacheKey = shared_lib_1.CacheKeyBuilder.build('workout-session', 'daterange', teamId, {
        ...
        const sessions = await this.workoutRepository.createQueryBuilder('workout')
            ...
            .getMany();
        // Cache for 5 minutes
        await this.cacheManager.set(cacheKey, sessions, 300, [`team:${teamId}`, 'workout-session:daterange']);
        return sessions;
    }
}
exports.CachedWorkoutSessionService = CachedWorkoutSessionService;
//# sourceMappingURL=CachedWorkoutSessionService.js.map