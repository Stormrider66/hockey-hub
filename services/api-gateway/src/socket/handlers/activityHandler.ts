...
export function registerActivityHandlers(socket: AuthenticatedSocket, _io: any) {
...
export function broadcastActivityEvent(io: any, activity: ActivityFeedEvent, organizationId: string) {
  ...
  switch (activity.visibility) {
    ...
    case 'private':
      ...
      // Optionally to target user if provided
      const targetUserId = (activity as any).target?.id || (activity.metadata as any)?.targetUserId;
      if (targetUserId) {
        io.to(`activity:user:${targetUserId}`).emit(SocketEventType.ACTIVITY_FEED_NEW, activity);
      }
      break;
  }
}
