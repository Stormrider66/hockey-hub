...
exports.CachedStatisticsService = void 0;
...
class CachedStatisticsService {
    constructor(dataSource, cacheManager) {
        ...
        this.playerPerformanceRepo = new repositories_1.CachedPlayerPerformanceRepository(this.dataSource.getRepository(entities_1.PlayerPerformanceStats), this.cacheManager);
        this.teamAnalyticsRepo = new repositories_1.CachedTeamAnalyticsRepository(this.dataSource.getRepository(entities_1.TeamAnalytics), this.cacheManager);
        this.workloadAnalyticsRepo = new repositories_1.CachedWorkloadAnalyticsRepository(this.dataSource.getRepository(entities_1.WorkloadAnalytics), this.cacheManager);
    }
    // Dashboard-specific optimized methods
    async getDashboardAnalytics(organizationId, teamId, playerId) {
        ...
        return this.cacheManager.getOrSet(cacheKey, async () => {
            ...
            promises.push(this.playerPerformanceRepo.getPerformanceAnalytics(organizationId, 'week'));
            // Get team analytics if teamId provided
            if (teamId) {
                promises.push(this.teamAnalyticsRepo.getTeamSeasonStats(teamId), this.workloadAnalyticsRepo.getTeamWorkloadSummary(teamId));
            }
            // Get player-specific data if playerId provided
            if (playerId) {
                promises.push(this.playerPerformanceRepo.getPlayerStats(playerId), this.workloadAnalyticsRepo.getPlayerWorkloadTrends(playerId, 4));
            }
            // Get high-risk players for any dashboard
            promises.push(this.workloadAnalyticsRepo.getHighRiskPlayers(organizationId));
    ...
    async getPlayerDashboardData(playerId) {
        const cacheKey = `player_dashboard:${playerId}`;
        return this.cacheManager.getOrSet(cacheKey, async () => {
            const [playerStats, workloadTrends, recentPerformance] = await Promise.all([
                this.playerPerformanceRepo.getPlayerStats(playerId, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000), // Last 30 days
                new Date()),
    ...
    async getCoachDashboardData(teamId) {
    ...
    async getPhysicalTrainerDashboardData(organizationId, teamId) {
        ...
        return this.cacheManager.getOrSet(cacheKey, async () => {
            ...
            if (teamId) {
                promises.push(this.workloadAnalyticsRepo.getWorkloadOptimizationSuggestions(teamId, 4), this.workloadAnalyticsRepo.getTeamWorkloadSummary(teamId));
            }
            const results = await Promise.all(promises);
            return {
                organizationId,
                ...
                recommendations: {
                    immediate: results[0]?.filter((p) => p.riskLevel === 'critical') || [],
                    weekly: teamId ? results[2]?.suggestions?.filter((s) => s.priority === 'high') || [] : []
                }
            };
...
exports.CachedStatisticsService = CachedStatisticsService;
//# sourceMappingURL=CachedStatisticsService.js.map