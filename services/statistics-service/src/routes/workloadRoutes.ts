...
export function createWorkloadRoutes(workloadRepo: CachedWorkloadAnalyticsRepository): Router {
  const router = Router();
  ...
  router.get('/teams/:teamId/summary',
    [
      ...
      query('weekStartDate').optional().isISO8601().withMessage('Week start date must be valid ISO8601 format')
    ],
    validateRequest,
    async (req: Request, res: Response) => {
      try {
        ...
        const weekStartDate = req.query.weekStartDate ? 
          new Date(req.query.weekStartDate as string) : undefined;
  ...
  router.get('/organizations/:organizationId/high-risk',
    [
      ...
      query('riskThreshold').optional().isFloat({ min: 0, max: 100 })
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const riskThreshold = req.query.riskThreshold ? 
          parseFloat(req.query.riskThreshold as string) : 70;
  ...
  router.post('/compare',
    [
      query('metric').optional().isIn(['totalWorkload', 'acuteChronicRatio', 'recoveryScore'])
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const metric = (req.query.metric as 'totalWorkload' | 'acuteChronicRatio' | 'recoveryScore') || 'totalWorkload';
  ...
  router.get('/teams/:teamId/optimization',
    [
      param('teamId').isUUID().withMessage('Valid team ID required'),
      query('weeks').optional().isInt({ min: 1, max: 12 }).withMessage('Weeks must be between 1 and 12')
    ],
    validateRequest,
    async (req: Request, res: Response) => {
      try {
  ...
  router.get('/alerts',
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const highRiskPlayers = await workloadRepo.getHighRiskPlayers(
          organizationId as string,
          severity === 'critical' ? 90 : severity === 'high' ? 75 : 60
        );
