...
export class PlayerTransferSaga {
  static definition(
    userServiceClient: UserServiceClient,
    // Add other service clients as needed
  ): SagaDefinition<PlayerTransferData> {
    const steps: SagaStep<PlayerTransferData>[] = [
      {
      ...
      },
      {
        ...
        compensate: async (data, context) => {
          ...
          if (context.metadata.fromTeamMembership) {
            await userServiceClient.addUserToTeam(
              data.fromTeamId,
              data.playerId,
              context.metadata.fromTeamMembership.role,
              context.metadata.fromTeamMembership.jerseyNumber,
              context.metadata.fromTeamMembership.position
            );
          }
        },
        retryable: true,
