...
export function parsePaginationParams(
  query: Partial<Record<string, unknown>> | PaginationOptions,
  ...
  const page = Math.max(1, toInt((query as any).page) || defaults.page || 1);
  const requestedLimit = toInt((query as any).limit) || defaults.limit || 20;
